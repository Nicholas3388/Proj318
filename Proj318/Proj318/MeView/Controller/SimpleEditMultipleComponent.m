                                ...
                                result:(TuSDKResult *)result;
{
    ...
    //    // 保存到临时文件
    _photoEditMultipleComponent.options.editFilterOptions.saveToTemp = NO;
...
#pragma mark - send image to ChangePortraitViewController
- (void)sendResult:(UIImage *)image {
    NSLog(@"Send result image");
    if ([_delegate respondsToSelector:@selector(sendResult:)]) {
        [_delegate sendResult:image];
    }
}

- (void)openEditorWithImage:(UIImage *)image {
    [self sendResult:image];
}
